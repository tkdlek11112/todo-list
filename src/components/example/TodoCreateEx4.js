...
import {PostCreate, PostCreateEx4} from "../../api";
...
function TodoCreateEx4() {
    ...
    const onSubmit = e => {
        e.preventDefault();
        console.log(value);
        const ret = PostCreateEx4(userId, value);
        ret.then((data) => {
            dispatch({type: 'CREATE', todo: {id: data.id, name: value, done: false}});
            setValue('');
            setOpen(false);
        });
    }
